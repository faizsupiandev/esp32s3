...
NoAudioCodecSpkOnly::NoAudioCodecSpkOnly(int output_sample_rate, gpio_num_t spk_bclk, gpio_num_t spk_ws, gpio_num_t spk_dout) {
    duplex_ = false;
    input_sample_rate_ = 16000;   // unused but AFE checks it; keep at 16kHz
    output_sample_rate_ = output_sample_rate;
    // rx_handle_ stays nullptr — Read() returns 0, EnableInput() is no-op
    ...
    ESP_ERROR_CHECK(i2s_channel_init_std_mode(tx_handle_, &std_cfg));
    ESP_LOGI(TAG, "SpkOnly channel created (no mic)");
}
